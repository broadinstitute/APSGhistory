...
def run_cycle_analysis(rundir,logdir,startcyc):
    cycpart = os.path.basename(rundir).split('_')[0]
    logfile = os.path.join(logdir, "realtimeAnalysis%s.%s.out" %
                           (dtstring(), cycpart))
    logfp = open(logfile,"w")
    logfp.write('\n'.join(['/seq/techdev/Solexa/scripts/solexa_cycle_by_cycle.pl',
                   '--firecrest='+rundir, '--startcycle='+str(startcyc) ]))
    retval = call(['/seq/techdev/Solexa/scripts/solexa_cycle_by_cycle.pl',
                   '--firecrest='+rundir, '--startcycle='+str(startcyc) ],
                  stdout=logfp,stderr=STDOUT,cwd=rundir)
    logfp.close()
    if retval == 0:
        return True
    return False
...
# run nabil's stuff
#/seq/techdev/Solexa/scripts/solexa_cycle_by_cycle.pl --firecrest=/zpool1/firecrest_test/analyzed/071005_SL-XAN_0005_FC9690/Data/C1-2_Firecrest1.8.28_18-10-2007_prodinfo/ --startcycle=2
