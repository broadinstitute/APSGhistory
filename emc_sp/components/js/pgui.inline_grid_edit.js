...
$.fn.sm_show_grid_error = function (a_options) {
    ...
    function ConstructErrorRow() {
        ...
        var errorPanel = $('<div>');
        errorPanel.addClass('grid_error_message').addClass('grid_message');
        errorPanel.append('<strong>' + options.errorMessageHeader + '</strong><br><br>');
        errorPanel.append(options.errorMessage);

        var closeButton = $('<div>');
        closeButton.addClass('grid_error_message_close_button');
        closeButton
            .sm_hoverclass('grid_error_message_close_button_hover')
    ...
    function construct() {
        ...
        grid.each(function () {
            $(this).find('tr#grid_header').before(ConstructErrorRow());
        });
        return _this;
    }
...
$.fn.sm_inline_grid_edit = function (a_options) {
    ...
    var defaults =
        {
            row: null,
            debug: false,
            editControlsContainer: 'td[data-column-name=InlineEdit]',
    ...
    function EmbedEditorFromXml(row, editorXmlElement, isInsert) {
        ...
        var dataCell = row.find('td[data-column-name=' + editorFieldName + ']');
        ...
        {
            PhpGen.InitializeEditorsController(isInsert ? PhpGen.DataOperation.Insert : PhpGen.DataOperation.Edit, inlineEditorContainer);
        });


        ProcessUnobtrusiveEditors(inlineEditorContainer);

        require([PhpGen.Module.PG.Autocomplete], function(autocomplete)
        {
            autocomplete.ProcessDependentAutocomplete(inlineEditorContainer);
        });
        ...
        function ProcessResponce() {
            ...
            if (responseXml.find('errormessage').length > 0) {
                ...
                responseXml.find('editor').each(function () {
                    var dataCell = currentRow.find('td[data-column-name=' + $(this).attr('name') + ']');
                    ReturnOldHtmlForCell(dataCell);
                ...
                responseXml.find('fieldvalue').each(function () {
                    ...
                    var dataCell = currentRow.children('td[data-column-name=' + $(this).attr('name') + ']');
    ...
    function CreateInlineInsertingControls(editControlsContainer) {
        if (options.useImagesForActions) {
            ...
            '<a href="#" class="inline_insert_cancel" title="' + options.cancelButtonHint + '"><img src="images/cancel.png" alt="' + options.cancelButtonHint + '"></a>' +
            '<a href="#" class="inline_insert_commit" title="' + options.commitButtonHint + '"><img src="images/ok.png" alt="' + options.commitButtonHint + '"></a>' +
            '</span>');
        }
        else {
            editControlsContainer.append(
    ...
    function construct() {
        ...
        else
        {
            ...
            editControlsContainers = grid.find('tr').children(options.editControlsContainer);
            editControlsContainers.each(function () {
                SetupEditControls($(this));
            });
        }
