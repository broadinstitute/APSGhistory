...
// PHPMaker 6 configuration for Table fsstat_history
$fsstat_history = NULL; // Initialize table object

// Define table class
class cfsstat_history {
	...
	function cfsstat_history() {
		$this->TableVar = "fsstat_history";
		$this->TableName = "fsstat_history";
		$this->SelectLimit = TRUE;
		$this->fsid = new cField('fsstat_history', 'x_fsid', 'fsid', "`fsid`", 19, -1, FALSE);
		$this->fields['fsid'] =& $this->fsid;
		$this->dirid = new cField('fsstat_history', 'x_dirid', 'dirid', "`dirid`", 19, -1, FALSE);
		$this->fields['dirid'] =& $this->dirid;
		$this->checked = new cField('fsstat_history', 'x_checked', 'checked', "`checked`", 19, -1, FALSE);
		$this->fields['checked'] =& $this->checked;
		$this->latest = new cField('fsstat_history', 'x_latest', 'latest', "`latest`", 16, -1, FALSE);
		$this->fields['latest'] =& $this->latest;
		$this->uid = new cField('fsstat_history', 'x_uid', 'uid', "`uid`", 19, -1, FALSE);
		$this->fields['uid'] =& $this->uid;
		$this->type = new cField('fsstat_history', 'x_type', 'type', "`type`", 19, -1, FALSE);
		$this->fields['type'] =& $this->type;
		$this->sumcnt = new cField('fsstat_history', 'x_sumcnt', 'sumcnt', "`sumcnt`", 21, -1, FALSE);
		$this->fields['sumcnt'] =& $this->sumcnt;
		$this->sumval = new cField('fsstat_history', 'x_sumval', 'sumval', "`sumval`", 20, -1, FALSE);
		$this->fields['sumval'] =& $this->sumval;
		$this->maxcnt = new cField('fsstat_history', 'x_maxcnt', 'maxcnt', "`maxcnt`", 21, -1, FALSE);
		$this->fields['maxcnt'] =& $this->maxcnt;
		$this->maxval = new cField('fsstat_history', 'x_maxval', 'maxval', "`maxval`", 20, -1, FALSE);
		$this->fields['maxval'] =& $this->maxval;
		$this->histogram = new cField('fsstat_history', 'x_histogram', 'histogram', "`histogram`", 205, -1, TRUE);
	...
	function HighlightName() {
		return "fsstat_history_Highlight";
	}
	...
	function SqlSelect() { // Select
		return "SELECT * FROM `fsstat_history`";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `fsstat_history` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		$SQL = "UPDATE `fsstat_history` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `fsstat_history` WHERE ";
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("fsstat_historyview.php", $this->UrlParm());
	}

	// Add url
	function AddUrl() {
		$AddUrl = "fsstat_historyadd.php";
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("fsstat_historyedit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("fsstat_historyadd.php", $this->UrlParm());
	}
	...
	// Delete url
	function DeleteUrl() {
		return $this->KeyUrl("fsstat_historydelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=fsstat_history" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
