...
// PHPMaker 6 configuration for Table complete_fs_view
$complete_fs_view = NULL; // Initialize table object

// Define table class
class ccomplete_fs_view {
	...
	function ccomplete_fs_view() {
		$this->TableVar = "complete_fs_view";
		$this->TableName = "complete_fs_view";
		$this->SelectLimit = TRUE;
		$this->id = new cField('complete_fs_view', 'x_id', 'id', "`id`", 3, -1, FALSE);
		$this->fields['id'] =& $this->id;
		$this->mount = new cField('complete_fs_view', 'x_mount', 'mount', "`mount`", 200, -1, FALSE);
		$this->fields['mount'] =& $this->mount;
		$this->path = new cField('complete_fs_view', 'x_path', 'path', "`path`", 200, -1, FALSE);
		$this->fields['path'] =& $this->path;
		$this->parent = new cField('complete_fs_view', 'x_parent', 'parent', "`parent`", 19, -1, FALSE);
		$this->fields['parent'] =& $this->parent;
		$this->deprecated = new cField('complete_fs_view', 'x_deprecated', 'deprecated', "`deprecated`", 16, -1, FALSE);
		$this->fields['deprecated'] =& $this->deprecated;
		$this->gid = new cField('complete_fs_view', 'x_gid', 'gid', "`gid`", 19, -1, FALSE);
		$this->fields['gid'] =& $this->gid;
		$this->group_name = new cField('complete_fs_view', 'x_group_name', 'group_name', "`group_name`", 200, -1, FALSE);
		$this->fields['group_name'] =& $this->group_name;
		$this->snapshot = new cField('complete_fs_view', 'x_snapshot', 'snapshot', "`snapshot`", 16, -1, FALSE);
		$this->fields['snapshot'] =& $this->snapshot;
		$this->tapebackup = new cField('complete_fs_view', 'x_tapebackup', 'tapebackup', "`tapebackup`", 16, -1, FALSE);
		$this->fields['tapebackup'] =& $this->tapebackup;
		$this->diskbackup = new cField('complete_fs_view', 'x_diskbackup', 'diskbackup', "`diskbackup`", 16, -1, FALSE);
		$this->fields['diskbackup'] =& $this->diskbackup;
		$this->type = new cField('complete_fs_view', 'x_type', 'type', "`type`", 19, -1, FALSE);
		$this->fields['type'] =& $this->type;
		$this->server_type = new cField('complete_fs_view', 'x_server_type', 'server_type', "`server_type`", 200, -1, FALSE);
	...
	function HighlightName() {
		return "complete_fs_view_Highlight";
	}
	...
	function SqlSelect() { // Select
		return "SELECT * FROM `complete_fs_view`";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `complete_fs_view` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		$SQL = "UPDATE `complete_fs_view` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `complete_fs_view` WHERE ";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[EW_PROJECT_NAME . "_" . $this->TableVar . "_" . EW_TABLE_RETURN_URL] <> "") {
			...
			return "complete_fs_viewlist.php";
		}
	}
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("complete_fs_viewview.php", $this->UrlParm());
	}

	// Add url
	function AddUrl() {
		$AddUrl = "complete_fs_viewadd.php";
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("complete_fs_viewedit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("complete_fs_viewadd.php", $this->UrlParm());
	}
	...
	// Delete url
	function DeleteUrl() {
		return $this->KeyUrl("complete_fs_viewdelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=complete_fs_view" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
