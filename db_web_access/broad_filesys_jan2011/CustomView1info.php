...
class cCustomView1 {
	...
	function cCustomView1() {
		...
		$this->id = new cField('CustomView1', 'x_id', 'id', "filesystem.id", 3, -1, FALSE);
		...
		$this->name = new cField('CustomView1', 'x_name', 'name', "server_type.name", 200, -1, FALSE);
	...
	function SqlSelect() { // Select
		return "SELECT filesystem.id, filesystem.mount, filesystem.path, filesystem.parent, filesystem.deprecated, filesystem.gid, filesystem.snapshot, filesystem.tapebackup, filesystem.diskbackup, server_type.name FROM server_type Inner Join filesystem On filesystem.type = server_type.id";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO server_type Inner Join filesystem On filesystem.type = server_type.id ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		$SQL = "UPDATE server_type Inner Join filesystem On filesystem.type = server_type.id SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM server_type Inner Join filesystem On filesystem.type = server_type.id WHERE ";
		$SQL .= EW_DB_QUOTE_START . 'id' . EW_DB_QUOTE_END . '=' .	ew_QuotedValue($rs['id'], $this->id->FldDataType) . ' AND ';
		$SQL .= EW_DB_QUOTE_START . 'name' . EW_DB_QUOTE_END . '=' .	ew_QuotedValue($rs['name'], $this->name->FldDataType) . ' AND ';
	...
	function SqlKeyFilter() {
		return "filesystem.id = @id@ AND server_type.name = '@name@'";
	}
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@name@", ew_AdjustSql($this->name->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if (!is_null($this->name->CurrentValue)) {
			$sUrl .= "&name=" . urlencode($this->name->CurrentValue);
		} else {
			return "javascript:alert('Invalid Record! Key is null');";
		}
		return $sUrl;
