...
{else}
    <a {n}
        href="{$PageNavigatorPage->GetPageLink()}" {n}
        class="page_link" {n}
        title="{$PageNavigatorPage->GetHint()}" {n}
        {if $PageNavigatorPage->HasShortCut()}
            pgui-shortcut="{$PageNavigatorPage->GetShortCut()}" {n}
        {/if}
        >{$PageNavigatorPage->GetPageCaption()}</a>
{/if}
{/foreach}
	...
	<table cellspacing="0" cellpadding="0" class="pgui-select-page-size">
		...
		<tr>
			<td>
				...
				<input {n}
					onkeyup="$('#custom_page_size_page_count_{$PageNavId}').html( GetPageCountForPageSize(this.value, {$PageNavigator->GetRowCount()}) )" {n}
