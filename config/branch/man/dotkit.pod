...
pathname to the file, leaving off the final I<.dk> suffix.)
...
and the "where",  and "rep" scripts referenced by the I<dk_where>,
and I<dk_rep> commands.

=head2 Logging

Find one other script at I<$DK_ROOT/etc/> named "log.example".  If
an executable file is located at I<$DK_ROOT/etc/log>, Dotkit
