...
class cusers {
	...
	function cusers() {
		...
		// uid
		$this->uid = new cField('users', 'users', 'x_uid', 'uid', '`uid`', 19, -1, FALSE, '`uid`', FALSE);
		...
		// gecos
		$this->gecos = new cField('users', 'users', 'x_gecos', 'gecos', '`gecos`', 200, -1, FALSE, '`gecos`', FALSE);
		$this->fields['gecos'] =& $this->gecos;

		// role
		$this->role = new cField('users', 'users', 'x_role', 'role', '`role`', 16, -1, FALSE, '`role`', FALSE);
		$this->role->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['role'] =& $this->role;

		// gid
		$this->gid = new cField('users', 'users', 'x_gid', 'gid', '`gid`', 3, -1, FALSE, '`gid`', FALSE);
		$this->gid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['gid'] =& $this->gid;

		// cobj
		$this->cobj = new cField('users', 'users', 'x_cobj', 'cobj', '`cobj`', 200, -1, FALSE, '`cobj`', FALSE);
	...
	function DeleteSQL(&$rs) {
		...
		$SQL .= ew_QuotedName('uid') . '=' . ew_QuotedValue($rs['uid'], $this->uid->FldDataType) . ' AND ';
   ...
   // Common render codes
		...
		$this->uid->CellCssStyle = ""; $this->uid->CellCssClass = "";
		$this->uid->CellAttrs = array(); $this->uid->ViewAttrs = array(); $this->uid->EditAttrs = array();
		...
		// gecos
		$this->gecos->CellCssStyle = ""; $this->gecos->CellCssClass = "";
		$this->gecos->CellAttrs = array(); $this->gecos->ViewAttrs = array(); $this->gecos->EditAttrs = array();

		// role
		$this->role->CellCssStyle = ""; $this->role->CellCssClass = "";
		$this->role->CellAttrs = array(); $this->role->ViewAttrs = array(); $this->role->EditAttrs = array();
		...
		// cobj
		$this->cobj->CellCssStyle = ""; $this->cobj->CellCssClass = "";
		$this->cobj->CellAttrs = array(); $this->cobj->ViewAttrs = array(); $this->cobj->EditAttrs = array();
